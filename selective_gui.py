...
# -*- coding: utf-8 -*-
"""
واجهة مستخدم رسومية للمعالجة الانتقائية
Selective Processing GUI Application
"""
...
class SelectiveProcessorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("معالج الصور الانتقائي - Selective Image Processor")
    ...
    def setup_ui(self):
        ...
        title_label = tk.Label(
            self.root, 
            text="معالج الصور الانتقائي - Selective Image Processor",
            font=("Arial", 16, "bold"),
            bg='#f0f0f0',
            fg='#2c3e50'
        )
        ...
        tk.Label(input_frame, text="مجلد الصور:", font=("Arial", 10)).pack(side=tk.LEFT)
        ...
        tk.Label(output_frame, text="مجلد الإخراج:", font=("Arial", 10)).pack(side=tk.LEFT)
        ...
        # صف خيارات الاختيار
        selection_frame = tk.Frame(settings_frame)
        selection_frame.pack(fill=tk.X, padx=10, pady=5)
        
        # عدد الصور المختارة
        tk.Label(selection_frame, text="عدد الصور المختارة:", font=("Arial", 10)).pack(side=tk.LEFT)
        self.max_images_var = tk.IntVar(value=5000)
        max_images_spinbox = tk.Spinbox(selection_frame, from_=1, to=50000, 
                                       textvariable=self.max_images_var, width=8)
        max_images_spinbox.pack(side=tk.LEFT, padx=5)
        
        # نمط الاختيار
        tk.Label(selection_frame, text="نمط الاختيار:", font=("Arial", 10)).pack(side=tk.LEFT, padx=(20,5))
        self.selection_mode = tk.StringVar(value="random")
        selection_combo = ttk.Combobox(selection_frame, textvariable=self.selection_mode, 
                                      values=["random", "first", "last", "pattern"], width=10)
        selection_combo.pack(side=tk.LEFT, padx=5)
        
        # نمط البحث
        tk.Label(selection_frame, text="نمط البحث:", font=("Arial", 10)).pack(side=tk.LEFT, padx=(20,5))
        self.pattern_var = tk.StringVar()
        pattern_entry = tk.Entry(selection_frame, textvariable=self.pattern_var, width=15)
        ...
        # صف الخيارات المتقدمة
        advanced_frame = tk.Frame(settings_frame)
        advanced_frame.pack(fill=tk.X, padx=10, pady=5)
        
        # عدد العمال
        tk.Label(advanced_frame, text="عدد العمال:", font=("Arial", 10)).pack(side=tk.LEFT)
        self.workers_var = tk.IntVar(value=4)
        workers_spinbox = tk.Spinbox(advanced_frame, from_=1, to=16, 
                                    textvariable=self.workers_var, width=5)
        workers_spinbox.pack(side=tk.LEFT, padx=5)
        
        # نوع هيكل الإخراج
        tk.Label(advanced_frame, text="هيكل الإخراج:", font=("Arial", 10)).pack(side=tk.LEFT, padx=(20,5))
        self.structure_var = tk.StringVar(value="flat")
        structure_combo = ttk.Combobox(advanced_frame, textvariable=self.structure_var, 
                                      values=["flat", "by_date", "by_name", "by_size"], width=10)
        ...
        notebook.add(available_frame, text="الصور المتاحة")
        
        # قائمة الصور المتاحة
        self.available_tree = ttk.Treeview(available_frame, columns=('size', 'date'), show='tree headings')
        self.available_tree.heading('#0', text='اسم الملف')
        self.available_tree.heading('size', text='الحجم')
        self.available_tree.heading('date', text='التاريخ')
        
        self.available_tree.column('#0', width=400)
        self.available_tree.column('size', width=100)
        self.available_tree.column('date', width=150)
        
        # شريط تمرير للصور المتاحة
        ...
        # تبويب الصور المختارة
        selected_frame = ttk.Frame(notebook)
        notebook.add(selected_frame, text="الصور المختارة")
        
        # قائمة الصور المختارة
        self.selected_tree = ttk.Treeview(selected_frame, columns=('size', 'date'), show='tree headings')
        self.selected_tree.heading('#0', text='اسم الملف')
        self.selected_tree.heading('size', text='الحجم')
        self.selected_tree.heading('date', text='التاريخ')
        
        self.selected_tree.column('#0', width=400)
        self.selected_tree.column('size', width=100)
        self.selected_tree.column('date', width=150)
        
        # شريط تمرير للصور المختارة
        selected_scrollbar = ttk.Scrollbar(selected_frame, orient=tk.VERTICAL, command=self.selected_tree.yview)
    ...
    def scan_images(self):
        ...
        if not input_path:
            messagebox.showerror("خطأ", "يرجى اختيار مجلد الصور")
            return
        ...
        # فحص الصور في thread منفصل
        thread = threading.Thread(target=self._scan_images_thread)
        thread.daemon = True
        thread.start()
    
    def _scan_images_thread(self):
        """فحص الصور في thread منفصل"""
        try:
            input_path = Path(self.input_var.get())
            
            # إنشاء معالج مؤقت للفحص
            temp_processor = SelectiveProcessor()
            
            # البحث عن الصور
            self.all_images = temp_processor.find_images_in_directory(
                input_path, 
                self.recursive_var.get()
            )
            
            # تحديث الواجهة
            self.root.after(0, self._update_available_images)
            
        except Exception as e:
            self.root.after(0, lambda: self._show_error(f"خطأ في فحص الصور: {str(e)}"))
    
    def _update_available_images(self):
        """تحديث قائمة الصور المتاحة"""
        # مسح القائمة السابقة
        ...
        for image_path in self.all_images:
            try:
                ...
                file_date = datetime.fromtimestamp(image_path.stat().st_mtime).strftime("%Y-%m-%d %H:%M")
                
                # تحويل الحجم إلى KB/MB
                if file_size < 1024:
                ...
                self.available_tree.insert('', 'end', text=image_path.name, 
                                         values=(size_str, file_date))
            except OSError:
                continue
        
        # تحديث الحالة
        self.status_label.config(text=f"تم العثور على {len(self.all_images)} صورة")
        self.select_btn.config(state=tk.NORMAL)
        
        # رسالة نجاح
        messagebox.showinfo("نجح", f"تم العثور على {len(self.all_images)} صورة")
    
    def select_images(self):
        """اختيار الصور للمعالجة"""
        if not self.all_images:
            messagebox.showwarning("تحذير", "يرجى فحص الصور أولاً")
            return
        
        # اختيار الصور في thread منفصل
        thread = threading.Thread(target=self._select_images_thread)
        thread.daemon = True
        thread.start()
    
    def _select_images_thread(self):
        """اختيار الصور في thread منفصل"""
        try:
            ...
            # إنشاء معالج مؤقت للاختيار
            temp_processor = SelectiveProcessor()
            
            if pattern:
                # اختيار بناءً على النمط
                self.selected_images = temp_processor.select_images_by_pattern(
                    Path(self.input_var.get()), pattern, self.recursive_var.get()
                )
            else:
                # اختيار بناءً على النمط المحدد
                if selection_mode == "random":
                    if len(self.all_images) > max_images:
                        self.selected_images = random.sample(self.all_images, max_images)
                    else:
                        self.selected_images = self.all_images.copy()
                elif selection_mode == "first":
                    self.selected_images = self.all_images[:max_images]
            ...
            # تحديث الواجهة
            self.root.after(0, self._update_selected_images)
            
        except Exception as e:
            self.root.after(0, lambda: self._show_error(f"خطأ في اختيار الصور: {str(e)}"))
    
    def _update_selected_images(self):
        """تحديث قائمة الصور المختارة"""
        # مسح القائمة السابقة
        ...
        for image_path in self.selected_images:
            try:
                ...
                self.selected_tree.insert('', 'end', text=image_path.name, 
                                        values=(size_str, file_date))
            except OSError:
                continue
        
        # تحديث الحالة
        self.status_label.config(text=f"تم اختيار {len(self.selected_images)} صورة")
        self.start_btn.config(state=tk.NORMAL)
        
        # رسالة نجاح
        messagebox.showinfo("نجح", f"تم اختيار {len(self.selected_images)} صورة للمعالجة")
    ...
    def start_processing(self):
        ...
        if not output_path:
            messagebox.showwarning("تحذير", "يرجى اختيار مجلد الإخراج")
            return
        
        # إنشاء معالج الصور الانتقائي
        self.processor = SelectiveProcessor(
    ...
    def _process_images_thread(self):
        ...
        try:
            ...
            self.log_message(f"بدء معالجة {len(self.selected_images)} صورة مختارة")
            results = self.processor.process_selected_images(
                self.selected_images,
                output_path,
    ...
    def update_statistics(self):
        ...
        stats_text = f"""
إحصائيات المعالجة الانتقائية
{'='*50}

إجمالي الصور المختارة: {stats['total_images']}
        ...
        for i, result in enumerate(self.results, 1):
            ...
            if result['status'] == 'success':
                ...
                if result.get('easyocr_results'):
                    stats_text += "   EasyOCR:\n"
                    for j, text_result in enumerate(result['easyocr_results'][:3], 1):
                        stats_text += f"     {j}. {text_result['text']} ({text_result['confidence']:.2%})\n"
                
                if result.get('tesseract_results'):
                    stats_text += "   Tesseract:\n"
                    for j, text_result in enumerate(result['tesseract_results'][:3], 1):
                        stats_text += f"     {j}. {text_result['text']} ({text_result['confidence']:.2%})\n"
            else:
...
def main():
    """تشغيل التطبيق"""
    root = tk.Tk()
    app = SelectiveProcessorGUI(root)
    root.mainloop()
